export class Webcam {
    ...
    async setup(mockVideoEl: HTMLVideoElement) {
        await new Promise<void>((resolve) => {
            ...
            mockVideoEl!.addEventListener('loadedmetadata', () =>
                resolve(), { once: true });
        });
    }
    ...
    public async run(
        ...
        try {
            ...
            if (now - this.lastFrameTime < 66) { // ~15 FPS
                ...
                canvasEl.toBlob(async (blob) => {
                    if (blob) {
                        ...
                        if (this.framesSent % 15 === 0) {
                            this.fps = Math.round(15 /
                                ((now - (this.lastFrameTime - 15 * 66)) / 1000));
                        }
                    }
                }, 'image/jpeg', 0.5); // Qualidade média para equilibrar velocidade/qualidade
            } catch (error) {
    ...
    public async streamFrames() {
        ...
        try {
            ...
            }, 'image/jpeg', 0.6); // Qualidade média para equilibrar velocidade/qualidade
